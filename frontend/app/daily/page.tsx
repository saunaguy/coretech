...
async function fetchDailyList(category: string): Promise<DailyTest[]> {
  try {
    const res = await fetch(`${API_BASE}/api/v1/daily/tests?category=${encodeURIComponent(category)}`, {
      cache: "no-store",
    });
    if (!res.ok) {
      return [];
    ...
    return data
      .filter((item): item is ApiDailyListItem & { id: string | number } => item.id !== undefined && item.id !== null)
      .map((item) => ({
        id: String(item.id),
...
async function fetchUserSolvedIds(): Promise<Set<string>> {
  try {
    ...
    return new Set(solved.map((id) => String(id)));
  } catch {
    return new Set();
  }
}
...
function rankItems(items: DailyTest[], solvedSet: Set<string>): RankedDailyTest[] {
  return items
    .map((item, idx) => ({
      ...item,
...
function normalizeTitle(item: RankedDailyTest, metaLabel?: string) {
  ...
  const prefix = CATEGORY_TITLE_PREFIX[item.category] ?? metaLabel ?? item.category;
  const base = prefix.endsWith("세트") ? prefix : `${prefix} 세트`;
  return `${base} ${indexLabel}`;
}

function TestItemCard({ item }: { item: RankedDailyTest }) {
  const createdLabel = safeDate(item.createdAt) ? format(safeDate(item.createdAt)!, "yyyy.MM.dd HH:mm") : null;
  const relativeLabel = formatRelative(item.createdAt);
  const difficultyClass = difficultyTone[item.difficulty ?? ""] ?? "border-slate-200 bg-slate-50 text-slate-600";
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2">
          <div className="flex flex-wrap items-center gap-2 text-xs font-medium tracking-wide text-muted-foreground">
            ...
            {createdLabel && relativeLabel && <span className="text-muted-foreground/70">|</span>}
            {relativeLabel && <span>{relativeLabel}</span>}
          </div>
          <div>
            <h3 className="text-lg font-semibold text-foreground group-hover:text-primary/90">{displayTitle}</h3>
            {meta && <p className="mt-1 text-sm text-muted-foreground">{meta.summary}</p>}
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <Badge variant="outline" className={cn("border text-xs font-medium", difficultyClass)}>
...
export default async function DailyPage() {
  ...
  const solvedSet = await fetchUserSolvedIds();
  const results = await Promise.all(categories.map((key) => fetchDailyList(key)));
  const hasAny = results.some((items) => items.length > 0);

  const fallback = fallbackTestsByCategory();
  const grouped = categories
    .map((category, index) => {
      const source = results[index].length > 0 ? results[index] : fallback[category] || [];
      const ranked = rankItems(source, solvedSet);
      return {
        category,
  ...
  const stats = {
    ...
    lastUpdated: latestDate ? formatDistanceToNow(latestDate, { addSuffix: true, locale: ko }) : "정보 없음",
  };
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      <section className="relative overflow-hidden rounded-3xl border bg-slate-950 text-white shadow-xl">
        ...
        <div className="absolute -top-32 right-0 h-64 w-64 rounded-full bg-primary/30 blur-3xl" aria-hidden />
        <div className="relative flex flex-col gap-8 p-10">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold tracking-widest text-primary/70">매일 한 문제</p>
            <h1 className="mt-2 text-4xl font-bold tracking-tight sm:text-5xl">매일 기술 퀴즈</h1>
            <p className="mt-4 text-base text-white/80">
              리눅스, 네트워크, 데이터베이스, 서버 운영 영역의 핵심 질문을 하루 한 번씩 확인하세요. 꾸준한 풀이가 실무 감각을 유지시켜 줍니다.
            </p>
          </div>

          <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
              <dt className="text-xs tracking-wide text-white/70">등록된 세트</dt>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
              <dt className="text-xs tracking-wide text-white/70">활성 카테고리</dt>
              <dd className="mt-2 flex items-center gap-2 text-2xl font-semibold">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
              <dt className="text-xs tracking-wide text-white/70">최근 업데이트</dt>
      ...
      {grouped.length === 0 ? (
        <section className="mx-auto flex max-w-3xl flex-col items-center justify-center rounded-3xl border bg-muted/40 px-8 py-16 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">API에서 문제를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</p>
        </section>
      ) : (
              ...
              <Card key={group.category} className="overflow-hidden border-border/80 shadow-lg">
                <CardHeader className="flex flex-col gap-4 border-b border-border/60 bg-muted/40 md:flex-row md:items-center md:justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl">{group.meta.label}</CardTitle>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {showMore && (
                      <Link href={`/daily/sets?category=${group.category}`} className="text-xs font-medium text-primary hover:underline">
                        전체 보기
                      </Link>
                    )}
                  </div>
      ...
      {!hasAny && (
        <p className="text-sm text-muted-foreground">
          현재 API 데이터가 없어 임시 목업 데이터를 기준으로 화면을 구성하고 있습니다.
        </p>
      )}
    </main>
  );
