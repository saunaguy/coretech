...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Append a log line. Default path: logs/YYYY-MM-DD/<agent>.jsonl or "
            "logs/YYYY-MM-DD/room-<room>/<agent>.jsonl"
        )
    )
    p.add_argument(
        "--agent",
        required=True,
        choices=["gpt", "gemini", "human", "system"],
        help="agent name",
    )
    p.add_argument(
        "--role",
        required=True,
        choices=["assistant", "user", "info", "error"],
        help="message role",
    )
    ...
    p.add_argument(
        "--room",
        default=None,
        help="optional room/topic name for grouping (creates logs/YYYY-MM-DD/room-<room>/)",
    )
